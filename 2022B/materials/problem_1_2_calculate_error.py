...
def find_b(alpha1,alpha2,num_c,true_b):
    ...
    for i in range(0,8):
        ...
        if cur_dif < min_dif:
            min_dif=cur_dif
            num_b=i+2
    print("推测值:",num_b,"真实值:",true_b)
    if num_b != true_b:
        ideal_alpha1=alpha_diagram[true_b-2][num_c-2][0]
        ideal_alpha2=alpha_diagram[true_b-2][num_c-2][1]
        return  get_dif(math.degrees(min_dif),
                        math.degrees(get_dif(ideal_alpha1,alpha1)+get_dif(ideal_alpha2,alpha2)))
    return 0
...
for i in range(sizes):
    ...
    # 使用蒙特卡洛模拟算法计算误差率
    info=(monte_carlo(dif[0],dif[1])) 
    dif=find_b(info[0],info[1],info[2],info[3])
    if dif == 0 :
        continue
    error+=1
    error_list.append(dif)
print(error_list)
print(error)
